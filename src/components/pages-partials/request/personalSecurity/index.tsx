import CircleIcon from '@mui/icons-material/Circle';
import RadioButtonUncheckedIcon from '@mui/icons-material/RadioButtonUnchecked';
import {
  Box,
  Checkbox,
  TableContainer,
  Table,
  TableBody,
  TableRow,
  TableCell,
  RadioGroup,
  FormControlLabel,
  Radio
} from '@mui/material';
import { DataGrid } from '@mui/x-data-grid';
import Image from 'next/image';
import React, { useState } from 'react';
import DatePicker from 'react-datepicker';

import DataGridTable from '@/components/RequestDataGrid/RequestDataGrid';

import {
  CalenderBox,
  DueDateTypography,
  RadioButtonHeading,
  RadioButtonSubHeading,
  ReportButtonBox,
  ReportCustomButton,
  RequestInputBox,
  SecondGridGenericBox,
  SecondGridMainBox,
  SubjectHeading
} from '../index.styles';
import Editor from '../jodit_editor';
import 'react-datepicker/dist/react-datepicker.css';
import { StyledBox } from '../meetingAttestation/index.styles';
import { HeaderSubTitle, HeaderTitle, HeaderWrapper } from '../newAdvisor/index.styles';
import { IRequestProps } from '../utils/interface';
import {
  DataGridContainer,
  NameCustomBox1,
  StyledHeadingWrapper,
  StyledSecurityQuaters,
  StyledSecurityQuatersBox
} from './index.styles';

const columns = [
  { field: 'date', headerName: 'Date', width: 150, headerClassName: 'custom-headerStyle' },
  { field: 'security', headerName: 'Security', width: 200, headerClassName: 'custom-headerStyle' },
  { field: 'buySell', headerName: 'BUY/Sell', width: 150, headerClassName: 'custom-headerStyle' },
  { field: 'price', headerName: 'Price', width: 150, headerClassName: 'custom-headerStyle' },
  { field: 'broker', headerName: 'Broker', width: 362, headerClassName: 'custom-headerStyle' }
];

const rows = [
  { id: 1, date: '2024-02-21', security: 'ABC', buySell: 'BUY', price: 100, broker: 'XYZ' },
  { id: 2, date: '2024-02-22', security: 'DEF', buySell: 'SELL', price: 150, broker: 'ABC' },
  { id: 3, date: '2024-02-23', security: 'GHI', buySell: 'BUY', price: 120, broker: 'DEF' }
];

const initialRows = [
  { id: 1, name: 'John', email: 'johnsmith@company.com', title: 'CEO' },
  { id: 2, name: 'Rachel Altman', email: 'rachel@company.com', title: 'COO' },
  { id: 3, name: 'Sarah Johnson', email: 'sarah@company.com', title: 'CMO' }
];

const MainColumns = [
  { field: 'name', headerName: 'Name', width: 250 },
  {
    field: 'email',
    headerName: 'Email',
    width: 300,
    cellClassName: () => {
      return 'email-cell';
    }
  },
  { field: 'title', headerName: 'Title', width: 400 }
];

export default function PersonalSecurityReportContent({
  content,
  setContent,
  title,
  subTitle
}: IRequestProps) {
  const [selectedDate, setSelectedDate] = useState<Date | null>(null); // Initialize selected date state
  const [showDatePicker, setShowDatePicker] = useState(false); // State to manage date picker visibility
  const [selectedValue, setSelectedValue] = useState('');
  const [row, setRow] = React.useState(initialRows);
  const [subject, setSubject] = useState<string>();

  const handleChange = (event: any) => {
    setSelectedValue(event.target.value);
  };

  const [selectedValue2, setSelectedValue2] = useState('');
  const handleChange2 = (event: any) => {
    setSelectedValue2(event.target.value);
  };

  const toggleDatePicker = () => {
    setShowDatePicker((prevState) => !prevState); // Toggle date picker visibility
  };

  const handleDateChange = (date: Date) => {
    setSelectedDate(date);
    setShowDatePicker(false); // Close the calendar after selecting a date
  };

  return (
    <div>
      <HeaderWrapper>
        <HeaderTitle>{title}</HeaderTitle>
        <HeaderSubTitle>{subTitle}</HeaderSubTitle>
      </HeaderWrapper>
      <SecondGridMainBox>
        <SubjectHeading>Subject</SubjectHeading>
        <Box>
          <RequestInputBox
            name="subject"
            type="text"
            value={subject}
            onChange={(e) => {
              setSubject(e.target.value);
            }}
          />
        </Box>

        <Editor content={content} setContent={setContent} />
      </SecondGridMainBox>
      <StyledBox>
        <StyledHeadingWrapper>
          <div>Personal Security Report</div>
          <div>Description</div>
        </StyledHeadingWrapper>
        {/* name date section */}
        <NameCustomBox1>
          <Box sx={{ flex: '61%', display: 'flex' }}>
            <label htmlFor="nameid">Name:</label>
            <input type="text" id="nameid" />
          </Box>
          <Box
            sx={{
              // flexGrow: 1,
              flex: '39%',
              display: 'flex',
              justifyContent: 'end',
              paddingRight: 10
            }}
          >
            <label htmlFor="nameid">Date:</label>
            <input type="date" id="nameid" style={{ width: '200px' }} />
          </Box>
        </NameCustomBox1>
        {/* radio button section */}
        <StyledSecurityQuatersBox>
          <StyledSecurityQuaters>
            <div>Quater Ending</div>
            <RadioGroup
              sx={{ display: 'flex', justifyContent: 'flex-start', width: '100%' }}
              row
              aria-label="options"
              name="options"
              value={selectedValue}
              onChange={handleChange}
            >
              <FormControlLabel
                value="March "
                control={
                  <Radio
                    sx={{
                      '&.Mui-checked': {
                        color: '#3A6FB6'
                      }
                    }}
                    icon={
                      <RadioButtonUncheckedIcon
                        sx={{
                          color: '#C3CAD2'
                        }}
                      />
                    }
                    checkedIcon={<CircleIcon />}
                  />
                }
                label="March "
              />
              <FormControlLabel
                value="June"
                control={
                  <Radio
                    sx={{
                      '&.Mui-checked': {
                        color: '#3A6FB6'
                      }
                    }}
                    icon={
                      <RadioButtonUncheckedIcon
                        sx={{
                          color: '#C3CAD2'
                        }}
                      />
                    }
                    checkedIcon={<CircleIcon />}
                  />
                }
                label="June"
              />
              <FormControlLabel
                value="September"
                control={
                  <Radio
                    sx={{
                      '&.Mui-checked': {
                        color: '#3A6FB6'
                      }
                    }}
                    icon={
                      <RadioButtonUncheckedIcon
                        sx={{
                          color: '#C3CAD2'
                        }}
                      />
                    }
                    checkedIcon={<CircleIcon />}
                  />
                }
                label="September"
              />
              <FormControlLabel
                value="December"
                control={
                  <Radio
                    sx={{
                      '&.Mui-checked': {
                        color: '#3A6FB6'
                      }
                    }}
                    icon={
                      <RadioButtonUncheckedIcon
                        sx={{
                          color: '#C3CAD2'
                        }}
                      />
                    }
                    checkedIcon={<CircleIcon />}
                  />
                }
                label="December"
              />
            </RadioGroup>
          </StyledSecurityQuaters>
          <StyledSecurityQuaters>
            <div>I have purchased Securities</div>
            <RadioGroup
              sx={{ display: 'flex', justifyContent: 'flex-start', width: '100%' }}
              row
              aria-label="options"
              name="options"
              value={selectedValue2}
              onChange={handleChange2}
            >
              <FormControlLabel
                value="Yes "
                control={
                  <Radio
                    sx={{
                      '&.Mui-checked': {
                        color: '#3A6FB6'
                      }
                    }}
                    icon={
                      <RadioButtonUncheckedIcon
                        sx={{
                          color: '#C3CAD2'
                        }}
                      />
                    }
                    checkedIcon={<CircleIcon />}
                  />
                }
                label="Yes "
              />
              <FormControlLabel
                value="No"
                control={
                  <Radio
                    sx={{
                      '&.Mui-checked': {
                        color: '#3A6FB6'
                      }
                    }}
                    icon={
                      <RadioButtonUncheckedIcon
                        sx={{
                          color: '#C3CAD2'
                        }}
                      />
                    }
                    checkedIcon={<CircleIcon />}
                  />
                }
                label="No"
              />
            </RadioGroup>
          </StyledSecurityQuaters>
        </StyledSecurityQuatersBox>
        {/* above table */}
        <DataGridContainer>
          <DataGrid
            columns={columns}
            rows={rows}
            hideFooterPagination
            hideFooter
            disableColumnFilter
            disableColumnMenu
            showColumnVerticalBorder
            showCellVerticalBorder
            autoHeight={true}
          />
        </DataGridContainer>
      </StyledBox>
      <SecondGridMainBox>
        <RadioButtonHeading>Assign Employees</RadioButtonHeading>
        <RadioButtonSubHeading>
          Marked people will receive an email notification
        </RadioButtonSubHeading>
        <DataGridTable rows={row} columns={MainColumns} />

        <SecondGridGenericBox>
          <CalenderBox>
            <DueDateTypography>
              {selectedDate ? selectedDate.toLocaleDateString() : 'Due Date'}
            </DueDateTypography>
            <Box onClick={toggleDatePicker}>
              <Image
                src={'/svgs/CalendarIcon.svg'}
                height={20}
                width={20}
                alt="calendar-icon"
                style={{ cursor: 'pointer' }}
              />
            </Box>
          </CalenderBox>
          {showDatePicker && (
            <DatePicker selected={selectedDate} onChange={handleDateChange} inline />
          )}
        </SecondGridGenericBox>

        <ReportButtonBox>
          <ReportCustomButton variant="contained">Send Request</ReportCustomButton>
        </ReportButtonBox>
      </SecondGridMainBox>
    </div>
  );
}
